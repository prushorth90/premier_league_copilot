...
Natural-language interpretation and response text sit behind provider-neutral interfaces. `ICoachMessageInterpreter` returns a normalized intent, fixture horizon, recommendation horizon, and candidate limit; `ICoachResponseGenerator` receives the original message, typed squad context, detected player, and complete structured orchestration result. The default `DeterministicCoachMessageInterpreter` and `DeterministicCoachResponseGenerator` require no external model. A future LLM adapter can replace either DI registration without changing React, SSE events, `FplCoachOrchestrator`, FPL tools, projection calculations, transfer validation, or `PlayerRecommendationService`.

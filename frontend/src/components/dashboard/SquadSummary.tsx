...
function PlayerCell({ pick }: { pick: FplSquadPick }) {
  return (
    <div className="relative min-w-0 bg-white px-4 py-4">
      ...
      <p className="truncate pr-6 text-sm font-bold">{pick.displayName}</p>
      <p className="mt-1 truncate text-xs text-black/45">{pick.teamName} · {pick.positionName}</p>
      <p className="mt-3 text-xs font-semibold text-[#287c50]">£{pick.price.toFixed(1)}m</p>
    </div>
  )
}